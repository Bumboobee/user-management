<?php
  ...
  try {
    ...
    //Prepared Statements
    $stmt = $conn->prepare('DELETE FROM usuario WHERE login=:login');
    $stmt->bindParam(':login', $login);
    $stmt->execute();
    
    if ($stmt->rowCount()==1){ //Excluiu com Sucesso
      ...
      $retornajs['erro'] = true;
      $retornajs['msg'] = '<div class="alert alert-danger" role="alert">Erro ao Excluir o Usuário.</div>';
      header('Content-Type: application/json');
      echo json_encode($retornajs);  //Retorna para o javascript
      return;        
    }
      ...
      $retornajs['erro'] = true;
      $retornajs['msg'] = '<div class="alert alert-danger" role="alert">Erro ao Excluir Usuário.'.$e->getMessage().'</div>'; 
      header('Content-Type: application/json');
      echo json_encode($retornajs);  //Retorna para o javascript
      return;
  }
